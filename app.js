(function() {
  ...
  function showIdentities(identities) {
    if (identities.length > 0) {
      ...
      for (const identity of identities) {
        ...
        readerKeyIdElement.innerText =
          `${identity.readerKeyId.substring(0, 4)} ${identity.readerKeyId.substring(4)}`;
        const readerKeyBlobElement = document.createElement('td');
        readerKeyBlobElement.className = 'ssh-key';
        readerKeyBlobElement.dataset.head = 'ssh-rsa ' + identity.readerKeyBlob
          .substr(0, identity.readerKeyBlob.length - 30);
        readerKeyBlobElement.dataset.tail = identity.readerKeyBlob.substr(-30);
        readerKeyBlobElement.innerHTML =
          `<span class="nowrap">ssh-rsa </span>${identity.readerKeyBlob}`;
        ...
        document.getElementsByClassName('identity-table-body')[0].appendChild(
          identityRow);
      }
    } else {
      document.getElementsByClassName('identity-table-body')[0].innerHTML =
        '<tr></tr>';
      document.getElementsByClassName('add-identity-message')[0].textContent =
        'Insert a smart card...';
    }
  }
  ...
  window.onload = async function() {
    document.getElementsByClassName('relay-options')[0].value =
      `--ssh-agent=${chrome.runtime.id}`;
    document.getElementsByClassName('relay-options')[0].onclick = function(
      e) {
      e.target.select();
    };
    ...
    try {
      ...
      document.getElementsByClassName('install-connector-step')[0].classList
        .remove('step-inapplicable');
